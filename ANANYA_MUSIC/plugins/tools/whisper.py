...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 7803657513]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💒 ᴡʜɪsᴘᴇʀ 💒",
            description=f"@TheAnanyaBot [USERNAME / ID] [ YOUR TEXT]",
            input_message_content=InputTextMessageContent(f"**📍ᴜsᴀɢᴇ:**\n\n@TheAnanyaBot (Target Username or ID) (Your Message).\n\n**Example:**\n@@qenxbot @WTF_NoHope I love You 😘"),
            thumb_url="https://files.catbox.moe/ynsu0c.jpg",
            reply_markup=switch_btn
        )
    ]
